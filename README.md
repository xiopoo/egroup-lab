...
hj/
├── .trae/
│   ├── skills/                      # Trae 内容生产 Skill（唯一真源，含 hj-* 与 dbs-*）
...
- 多领域内容目录：`content-system/content/content-catalog.json`；
- 微信与 Markdown 成品：`content-system/output/`；
- 运营总览：`content-system/准备工具.md`。

内容生产和官网使用同一仓库，但按领域隔离。灰金内容使用 `hj-*` 流程，巴芒研究使用 `dbs-*` 流程；跨域文章必须标注来源并经过人工确认。网站文章目前不是由内容系统自动同步，任何“自动同步”开发都必须单独立项和验证。

...
description: |
  内容结构化系统。当用户想把本地大量文稿、推文、选题、案例做成可重组的内容系统，或提到「内容资产工程化」「主题地图」「选题装配」「把旧内容变成可复用资产」时调用。
...
如果用户只是想改一篇内容，转到内容创作诊断（dbs-content）、短视频开头优化（dbs-hook）、小红书标题优化等轻量技能。
...
- 好问题技能（dbs-good-question）已把问题说明书写清楚，且适合自动化执行
- Agent 迁移技能（dbs-agent-migration）已经把 Agent 工作台迁好，下一步要搭内容工程
- 用户明确需要本地内容资产长期工程化

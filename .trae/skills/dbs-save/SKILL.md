---
name: dbs-save
description: 诊断状态保存。当用户说「保存」「记下来」「存档」「这个结论留着」、想把当前诊断的关键状态保存到本地以便下次恢复时调用。
---
...
| 说「记下来 XXX」并指定了标题（比如「记下来 卖什么没想清楚」） | 用户指定标题 |
...
**用户显式指定**：用「项目名」参数，例如说「存到 my-project 项目下，卖什么没想清楚」。
...
> 现在没什么可存的——前面没做过诊断。先用商业诊断或别的诊断 skill 走一轮，再来存档。
...
如果对话里推荐了下一步 skill（比如诊断末尾推荐了做对标分析），就填上。
如果没有明显推荐，留空。

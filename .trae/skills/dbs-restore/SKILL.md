---
name: dbs-restore
description: 诊断状态恢复。当用户说「上次」「之前的」「接着」「续上」「上次诊断到哪了」、想接续上次的诊断继续时调用。拉出最近一份存档。
---
...
1. 用户给了序号 → 列出当前项目下所有存档按文件名排序，取第 N 条
...
```
还没有任何诊断记录。先用商业诊断 skill 做一次，再存下来。下次就能说「接着上次」接续。
```

**情况 B：list 模式**

转给保存工具的 list 逻辑（输出格式一致）。
...
| 「我换个问题」/「重新开始」 | 默认走商业诊断 |
| 提到具体的新需求（小红书标题、AI 检测等） | 按主路由判断，路由到对应 skill |
